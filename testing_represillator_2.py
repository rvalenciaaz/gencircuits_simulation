...
# Load the C library
c_lib = ctypes.CDLL('./libRepressilator_2.so')
num_points = 1000
...
c_lib.simulate_repressilator.restype = ctypes.POINTER(ctypes.c_double * 5 * num_points)

# Prepare data for the C function
params_array = np.array([1, 1, 1, 0.01, 10, 2], dtype=np.float64)  # deg_m, deg_p,alpha, alpha_0, beta, n
initial_conditions = np.array([0.0, 1.0, 0.0, 2.0], dtype=np.float64)  # m1, m2, m3, p1, p2, p3
...
plt.tight_layout()
#plt.show()
plt.savefig("repressilator_variables_2.jpg", bbox_inches="tight", dpi=300)
plt.savefig("repressilator_variables_2.png", bbox_inches="tight", dpi=300, transparent=True)
