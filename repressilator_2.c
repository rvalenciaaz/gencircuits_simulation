...
int repressilator_func(double t, const double y[], double f[], void *params) {
    ...
    f[0] = -deg_m * y[0] + alpha / (1 + pow(y[3], n)) + alpha_0; 
    f[1] = -deg_m * y[1] + alpha / (1 + pow(y[2], n)) + alpha_0; 

    f[2] = -deg_p * y[2] + beta * y[0]; 
    f[3] = -deg_p * y[3] + beta * y[1];
...
double* simulate_repressilator(double *params_array, double *initial_conditions, double *time_span, double num_points) {
    ...
    gsl_odeiv2_system sys = {repressilator_func, NULL, 4, params_array};
    ...
    double* results = malloc(numi * 5 * sizeof(double));
    if (!results) {
    ...
    for (i = 0; i < numi; i=i+1) {
        ...
        results[(i * 5) + 0] = ti;
        results[(i * 5) + 1] = initial_conditions[0];
        results[(i * 5) + 2] = initial_conditions[1];
        results[(i * 5) + 3] = initial_conditions[2];
        results[(i * 5) + 4] = initial_conditions[3];
    }
